...
void NavigationController::peopleflagCB(const std_msgs::Float64MultiArray::ConstPtr& people_state){
	if (people_state->data[1]>0.5){
		people_distance = people_state->data[2];
		people_in=true;
		out_counter=0;
	}
	else if(out_counter >=4){
		people_in=false;
		out_counter=0;
	}
	else
		out_counter++;
	//cout << people_state->data[1] << endl ;
	//ROS_ERROR("%d ==============================================  %.2f",out_counter,people_state->data[1]);
}
...
void NavigationController::run()
{
   ...
   //                                << q.z() << " ," << q.w() << std::endl;
	...
	//0909
		...
		if(people_in == true) cmd.linear.x = cmd.linear.x *0.4;
		else cmd.linear.x=cmd.linear.x;
