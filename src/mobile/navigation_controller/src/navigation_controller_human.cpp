...
public:
	...
	void run();
	//people
	void peopleflagCB(const std_msgs::Float64MultiArray::ConstPtr& people_state);
...
const double NavigationController::MaxRobotLinearVelocity = 0.5; //0.5		// m/s
const double NavigationController::MaxRobotAngularVelocity = 60.0 * D2R; //40	// radian/s
...
NavigationController::NavigationController(tf::TransformListener& tf) : tf_(tf)
{
	...
	//people
	people_sub = nh_.subscribe("people_camera", 1, &NavigationController::peopleflagCB, this);
...
//people
void NavigationController::peopleflagCB(const std_msgs::Float64MultiArray::ConstPtr& people_state){
	if (people_state->data[1]>1.5){
		people_distance = people_state->data[2];
		people_in=true;
		out_counter=0;
		cout<<"111111"<<"\n";
	}
	else if(out_counter >=10){
		people_in=false;
		out_counter=0;
		cout<<"2222222222"<<"\n";
	people_in=false;
	...
	cout <<"people_state->data[1] :"<< people_state->data[1] << endl ;
	ROS_ERROR("out_counter %d ============================================== people_state->data[1] %.2f",out_counter,people_state->data[1]);
}
...
void NavigationController::run()
{
	...
	std::ofstream out_0("/home/yu/Documents/mobile/src/navigation_controller/cmd_txt/cmd666.txt");
   ...
   //                                << q.z() << " ," << q.w() << std::endl;
	...
	//0909
		...
		cout<<"people in:"<<people_in<<"\n";
		cout<<"people_distance:"<<people_distance<<"\n";
		...
		if(state_ != FINISH ){
			...
			global_flag_past=true;
			
		}
		else{
			nav_status.data = "false";
			global_flag_past=false;
		}
		...
		if( global_flag != global_flag_past){
			c++;
			
		}	
		else{
			global_flag = global_flag_past;
	
		}

		cout<<"false_to_true_times:"<< c <<"\n";
	


		if( state_ != FINISH && c >= 165 ){
			cout<<"peoplepeoplepeoplepeoplepeoplepeople"<<"\n";

			if(people_in && people_distance > 0.3){
			...
			else if(people_in && people_distance>0.247 && people_distance < 0.3){
				cmd.linear.x = cmd.linear.x*0.6;
				cmd.angular.z = 0;
				cout<<"slow!!!!!!!!!!!!!!"<<"\n";
			}
		
			else if(people_in && people_distance<=0.247){
				cmd.linear.x = 0;
				cmd.angular.z = 0;
				cout<<"stop!!!!!!!!!!!!!!"<<"\n";
			}
		...
		last_cmd_ = cmd;
		cout<<"runrunrunrunrunrunrunrunrunrunrun"<<"\n";
