	...
	while(ros::ok()){
		...
		while(1){
			if(serR.available() >= 8){
				...
				array.data.clear();
				//read 8 unsigned char
				serR.read(array.data,8);
				for(int i = 0; i < 4; i++){
					rencoder[i] = array.data[i+2];
				}
				//std::cout<<"RightEncoderInfo=" << (int)(unsigned int)array.data[0] <<" ;" <<(int)(unsigned int)array.data[1] <<" ;" <<(int)(unsigned int)array.data[2] <<" ;" << (int)(unsigned int)array.data[3] <<" ;"<<(int)(unsigned int)array.data[4] <<" ;" <<(int)(unsigned int)array.data[5] << "\n";
				if(initial[1] == true){
					//left initial value
					for(int i = 0; i < 4; i++){
