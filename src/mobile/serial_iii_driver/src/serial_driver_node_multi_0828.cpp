...
const double WheelDistance = 0.413;		//m tuned
...
int main (int argc, char** argv)
{
	...
	while(ros::ok()){
		...
		//std::cout << serR.available() << std::endl;
		while(1){
			
			if(serR.available() >= 1){
				// ROS_ERROR("FUCK3");	
				// ROS_INFO_STREAM("Reading from serial port");
				ROS_INFO_STREAM_THROTTLE(1.0, "Reading from serial port"
                                        << std::endl);
				array_.data.clear();
        ...
        unsigned int CRCMotorRcd[6] = { 0x01,array_.data[0],array_.data[1],array_.data[2],array_.data[3], array_.data[4]};
				...
				if((array_.data[5]==(CRC&0XFF))&&(array_.data[6]==(CRC>>8))&&(array_.data[13]==(CRC2&0XFF))&&(array_.data[14]==(CRC2>>8))){
					...
					for(int i = 0; i < 4; i++){
						lencoder[i] = array_.data[i+1];
						rencoder[i] = array_.data[i+9];
					}			
					break;
				}	
				else 
		...
		double WV[2] = { 0.0, 0.0 };
		WV[0] = ((double)WPluse[0] / EncoderPlus) * 0.253 * (2.0 * M_PI * WheelRadius) / GearRatio / DriverSampleTime;
		WV[1] = ((double)WPluse[1] / EncoderPlus) * 0.253 * (2.0 * M_PI * WheelRadius) / GearRatio / DriverSampleTime;
