...
class HdlLocalizationNodelet_isci : public nodelet::Nodelet {
public:
  using PointT = pcl::PointXYZI;

  HdlLocalizationNodelet_isci() : tf_buffer(), tf_listener(tf_buffer) {
  }
  virtual ~HdlLocalizationNodelet_isci() {
  }
  ...
  void onInit() override {
    ...
    if (use_imu) {
      ...
      imu_sub = mt_nh.subscribe("/gpsimu_driver/imu_data", 256, &HdlLocalizationNodelet_isci::imu_callback, this);
    }
    points_sub = mt_nh.subscribe("/velodyne_points", 5, &HdlLocalizationNodelet_isci::points_callback, this);
    globalmap_sub = nh.subscribe("/globalmap", 1, &HdlLocalizationNodelet_isci::globalmap_callback, this);
    initialpose_sub = nh.subscribe("/initialpose", 8, &HdlLocalizationNodelet_isci::initialpose_callback, this);
    ...
    if(use_global_localization) {
      ...
      relocalize_server = nh.advertiseService("/relocalize", &HdlLocalizationNodelet_isci::relocalize, this);
    }
  }
...
private:
  ...
  void publish_odometry(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    /* 假設有自己的odom，代表velodyne和odom有相對關係，所以把odom2map當作估測結果 */
    if(tf_buffer.canTransform(robot_odom_frame_id, odom_child_frame_id, ros::Time(0))) {
      // map_wrt_frame是map2velodyne的轉移矩陣（這個是估測來的，就是函式的pose變數）
      ...
      map_wrt_frame.child_frame_id = "odom";

      // odom_child_frame_id: velodyne, robot_odom_frame_id: odom
      // frame_wrt_odom是velodyne2odom的轉移矩陣
      ...
      tf2::doTransform(map_wrt_frame, map_wrt_odom, frame_wrt_odom); // map2velodyne * velodyne2odom = map2odom 轉移矩陣:map_wrt_odom

      // 轉回odom2map（這樣tf tree才能連上）
    ...
    /* 若沒有自己的odom，則把velodyne2map當作估測結果 */
    else {
    ...
    pose_pub.publish(odom); //這裡發布velodyne2map的/odom topic
  }
...
PLUGINLIB_EXPORT_CLASS(hdl_localization::HdlLocalizationNodelet_isci, nodelet::Nodelet)