...
class GlobalmapServerNodelet_isci : public nodelet::Nodelet {
public:
  using PointT = pcl::PointXYZI;

  GlobalmapServerNodelet_isci() {
  }
  virtual ~GlobalmapServerNodelet_isci() {
  }
  ...
  void onInit() override {
    ...
    map_update_sub = nh.subscribe("/map_request/pcd", 10,              &GlobalmapServerNodelet_isci::map_update_callback, this);

    globalmap_pub_timer = nh.createWallTimer(ros::WallDuration(1.0), &GlobalmapServerNodelet_isci::pub_once_cb, this, true, true);
  }
...
private:
  void initialize_params() {
    ...
    // // for global map filtering
    // double z_min = 0.3;
    // double z_max = 2.5;

    // pcl::PointCloud<PointT>::Ptr distance_filtered(new pcl::PointCloud<PointT>());
    // distance_filtered->reserve(globalmap->size());
    // std::copy_if(globalmap->begin(), globalmap->end(), std::back_inserter(distance_filtered->points), [&](const PointT& p) {
    //   return p.z > z_min && p.z < z_max; // 設定點雲允許存在的距離範圍
    // });
    // distance_filtered->width = distance_filtered->size();
    // distance_filtered->height = 1;
    // distance_filtered->is_dense = false;
    // distance_filtered->header = globalmap->header;
    // globalmap = distance_filtered;
...
PLUGINLIB_EXPORT_CLASS(hdl_localization::GlobalmapServerNodelet_isci, nodelet::Nodelet)
