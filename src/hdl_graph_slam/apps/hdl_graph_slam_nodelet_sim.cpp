...
public:
  ...
  virtual void onInit() {
    ...
    // published_odom_topic = private_nh.param<std::string>("published_odom_topic", "/estimated_odom");
    map_frame_id = private_nh.param<std::string>("map_frame_id", "map");
    odom_frame_id = private_nh.param<std::string>("odom_frame_id", "odom");
    // odom_frame_id = private_nh.param<std::string>("odom_frame_id", "estimated_odom");
    ...
    // subscribers
    /* 使用message filter，訂閱兩個topic: odom, filtered_point，並且只使用一個callback function */
    ...
    sync->registerCallback(boost::bind(&HdlGraphSlamNodelet_sim::cloud_callback, this, _1, _2));
    imu_sub = nh.subscribe("/gpsimu_driver/imu_data", 1024, &HdlGraphSlamNodelet_sim::imu_callback, this);
    floor_sub = nh.subscribe("/floor_detection/floor_coeffs", 1024, &HdlGraphSlamNodelet_sim::floor_coeffs_callback, this);

    if(private_nh.param<bool>("enable_gps", true)) {
      gps_sub = mt_nh.subscribe("/gps/geopoint", 1024, &HdlGraphSlamNodelet_sim::gps_callback, this);
      nmea_sub = mt_nh.subscribe("/gpsimu_driver/nmea_sentence", 1024, &HdlGraphSlamNodelet_sim::nmea_callback, this);
      navsat_sub = mt_nh.subscribe("/gps/navsat", 1024, &HdlGraphSlamNodelet_sim::navsat_callback, this);
    }
    ...
    load_service_server = mt_nh.advertiseService("/hdl_graph_slam/load", &HdlGraphSlamNodelet_sim::load_service, this);
    dump_service_server = mt_nh.advertiseService("/hdl_graph_slam/dump", &HdlGraphSlamNodelet_sim::dump_service, this);
    save_map_service_server = mt_nh.advertiseService("/hdl_graph_slam/save_map", &HdlGraphSlamNodelet_sim::save_map_service, this);

    graph_updated = false;
    double graph_update_interval = private_nh.param<double>("graph_update_interval", 3.0);
    double map_cloud_update_interval = private_nh.param<double>("map_cloud_update_interval", 10.0);
    optimization_timer = mt_nh.createWallTimer(ros::WallDuration(graph_update_interval), &HdlGraphSlamNodelet_sim::optimization_timer_callback, this);
    map_publish_timer = mt_nh.createWallTimer(ros::WallDuration(map_cloud_update_interval), &HdlGraphSlamNodelet_sim::map_points_publish_timer_callback, this);
  }
...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    Eigen::Isometry3d odom = odom2isometry(odom_msg); //把type為nav_msgs::Odometry的資訊，轉換成velodyne相對於odom的轉移矩陣
    ...
    if(!keyframe_updater->update(odom)) { //判斷是否更新keyframe(這裡是若不需更新要做的)
      // std::cout << "no update keyframe!!!" << std::endl;
      std::lock_guard<std::mutex> lock(keyframe_queue_mutex); //用法：對keyframe_queue_mutex互斥鎖進行了鎖定，並在該區塊作用域結束時自動解鎖
    ...
    double accum_d = keyframe_updater->get_accum_distance(); //和第一個keyframe的累積距離
    KeyFrame::Ptr keyframe(new KeyFrame(stamp, odom, accum_d, cloud));

    std::lock_guard<std::mutex> lock(keyframe_queue_mutex);
    keyframe_queue.push_back(keyframe); //新增keyframe
  }
  ...
  bool flush_keyframe_queue() {
    ...
    // 因為使用輪速里程計，所以odom2map相對關係幾乎不變
    Eigen::Isometry3d odom2map(trans_odom2map.cast<double>()); //這裡會擋有keyframe的時候，有keyframe才會通過這段轉移矩陣宣告
    // std::cout << "odom2map matrix:" << std::endl;
    // std::cout << odom2map.matrix() << std::endl;
    ...
    for(int i = 0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {
      ...
      // add pose node
      Eigen::Isometry3d odom = odom2map * keyframe->odom; //keyframe在odom frame的姿態，轉換至相對map frame的姿態
      // std::cout << "odom matrix:" << std::endl;
      // std::cout << odom.matrix() << std::endl;
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    // std::cout << "optimization_timer_callback" << std::endl;
...
PLUGINLIB_EXPORT_CLASS(hdl_graph_slam::HdlGraphSlamNodelet_sim, nodelet::Nodelet)
