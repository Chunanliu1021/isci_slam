...
class PrefilteringNodelet_sim : public nodelet::Nodelet {
public:
  typedef pcl::PointXYZI PointT;

  PrefilteringNodelet_sim() {}
  virtual ~PrefilteringNodelet_sim() {}
  ...
  virtual void onInit() {
    ...
    if(private_nh.param<bool>("deskewing", false)) {
      imu_sub = nh.subscribe("/imu/data", 1, &PrefilteringNodelet_sim::imu_callback, this);
    }

    points_sub = nh.subscribe("/velodyne_points", 64, &PrefilteringNodelet_sim::cloud_callback, this);
...
PLUGINLIB_EXPORT_CLASS(hdl_graph_slam::PrefilteringNodelet_sim, nodelet::Nodelet)
