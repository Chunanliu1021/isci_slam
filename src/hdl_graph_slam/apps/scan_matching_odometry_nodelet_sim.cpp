...
class ScanMatchingOdometryNodelet_sim : public nodelet::Nodelet {
public:
  typedef pcl::PointXYZI PointT;
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  ScanMatchingOdometryNodelet_sim() {}
  virtual ~ScanMatchingOdometryNodelet_sim() {}
  ...
  virtual void onInit() {
    ...
    if(private_nh.param<bool>("enable_imu_frontend", false)) {
      msf_pose_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/msf_core/pose", 1, boost::bind(&ScanMatchingOdometryNodelet_sim::msf_pose_callback, this, _1, false));
      msf_pose_after_update_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/msf_core/pose_after_update", 1, boost::bind(&ScanMatchingOdometryNodelet_sim::msf_pose_callback, this, _1, true));
    }

    points_sub = nh.subscribe("/filtered_points", 256, &ScanMatchingOdometryNodelet_sim::cloud_callback, this);
...
private:
  ...
  void initialize_params() {
    ...
    // odom_frame_id = pnh.param<std::string>("odom_frame_id", "estimated_odom");
   ...
   */
  // cloud_msg: /filtered_points, in base_link frame
  ...
  void publish_odometry(const ros::Time& stamp, const std::string& base_frame_id, const Eigen::Matrix4f& pose) {
    ...
    // 這裡可能影響到模擬環境和SLAM的base_link tf衝突
...
PLUGINLIB_EXPORT_CLASS(hdl_graph_slam::ScanMatchingOdometryNodelet_sim, nodelet::Nodelet)
