...
namespace gazebo
{
  class PlannarMover_Desk : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(
          std::bind(&PlannarMover_Desk::OnUpdate, this));
      
      this->old_secs =ros::Time::now().toSec();
      
      // Create a topic name
      std::string plannar_pos_topicName = "/obsvel_plannar_Desk";
      ...
      if (!ros::isInitialized())
      {
        int argc = 0;
        char **argv = NULL;
        ros::init(argc, argv, "plannar_Desk",
            ros::init_options::NoSigintHandler);
      }
         
      // Create our ROS node. This acts in a similar manner to
      // the Gazebo node
      this->rosNode.reset(new ros::NodeHandle("plannar_Desk_rosnode"));
      ...
      ros::SubscribeOptions so =
        ros::SubscribeOptions::create<geometry_msgs::Twist>(
            ...
            1,
            boost::bind(&PlannarMover_Desk::OnRosMsg_Pos, this, _1),
            ros::VoidPtr(), &this->rosQueue);
      this->rosSub = this->rosNode->subscribe(so);
      
      // Spin up the queue helper thread.
      this->rosQueueThread =
        std::thread(std::bind(&PlannarMover_Desk::QueueThread, this));

      ROS_WARN("Loaded PlannarMover_Desk Plugin with parent...%s, only X Axis Freq Supported in this V-1.0", this->model->GetName().c_str());
  ...
  GZ_REGISTER_MODEL_PLUGIN(PlannarMover_Desk)
}