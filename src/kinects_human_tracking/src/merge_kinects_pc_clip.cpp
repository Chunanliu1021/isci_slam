...
void merge_pcs(vector<PCMsg::ConstPtr> pcs_msg_pc2){
  ...
  for(int i=0; i<pcs_msg_pc2.size(); i++){
    ...
    // range save point, remove x:-0.25-0.25, y: 0-(-0.7) data

    // for (int kps = 0; kps < pcs_[i]->points.size(); kps++)
    // {
    //   if ((pcs_[i]->points[kps].y<0.3&&(pcs_[i]->points[kps].y)>-0.3)&&((pcs_[i]->points[kps].x)<0&&(pcs_[i]->points[kps].x)>-0.8))
    //   {
    //     // pcs_[i]->points.erase((pcs_[i]->points.begin()+kps));
    //     pcs_[i]->points[kps].x=10;
    //     pcs_[i]->points[kps].y=10;
    //     pcs_[i]->points[kps].z=10;
    //     pcs_[i]->points[kps].r=255;
    //     pcs_[i]->points[kps].g=0;
    //     pcs_[i]->points[kps].b=0;
    //   }else{
    //     pcs_[i]->points[kps].r=0;
    //     pcs_[i]->points[kps].g=255;
    //     pcs_[i]->points[kps].b=0;
    //   }
      
    // }
    // pcl::PassThrough<pcl::PointXYZRGB> pass;
    // pass.setInputCloud (pcs_[i]);
    // pass.setFilterFieldName ("y");
    // pass.setFilterLimits (-0.25, 0.25);
    // pass.setFilterLimitsNegative (true);
    // pass.filter (*pcs_[i]);
    // pass.setFilterFieldName ("x");
    // pass.setFilterLimits (-0.8, 0.0);
    // pass.setFilterLimitsNegative (true);
    // pass.filter (*pcs_[i]);
    // pass.setFilterFieldName ("z");
    // pass.setFilterLimits (-0.8, 20.0);
    // pass.setFilterLimitsNegative (false);
    // pass.filter (*pcs_[i]);
    
    pcl::CropBox<pcl::PointXYZRGB> clipper;//要包含的头文件，在官方文档里
    Eigen::Vector4f max_point;
    Eigen::Vector4f min_point;
    min_point<<-1.0,-0.3, -0.4,1.0;//cube的两个对角点
    max_point<< 0.0, 0.3,  0.5,1.0;

    clipper.setMin(min_point);
    clipper.setMax(max_point);
    clipper.setInputCloud(pcs_[i]);
    clipper.setNegative(true);//默认为false
    clipper.filter(*pcs_[i]);
